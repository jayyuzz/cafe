...
export default function KDSPage() {
  ...
  useEffect(() => {
    ...
    // Subscribe ke Realtime Supabase (tanpa interval/polling)
  ...
  const renderOrderCard = (order: OrderWithItems) => {
    ...
    return (
      <div key={order.id} className={"bg-card border shadow-sm rounded-lg flex flex-col transition-all hover:shadow-md h-full"}>
        <div className={\px-4 py-2 flex justify-between items-center text-primary-foreground \\`}>
          ...
          <ul className={"space-y-3"}>
            {order.order_items.map(item => {
              ...
              return (
                <li 
                  key={item.id} 
                  className={\lex gap-2 text-sm p-2 rounded-md cursor-pointer transition-all hover:bg-muted/50 \\`}
